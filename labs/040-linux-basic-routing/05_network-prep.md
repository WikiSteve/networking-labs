...
Configure both computers to match the topology diagram using static IP assignments. On Debian, you can configure persistent static IPs by editing **`/etc/network/interfaces`**. Alternatively, you can use the **`ip`** command, but remember that those changes are temporary and will not persist after a reboot. Set the server interface to **`.200`** (e.g., **`192.168.90.200`**) and the client interface to **`.201`** (e.g., **`192.168.90.201`**) matching your specific NAT network. Set the client's default gateway to point to the server (**`.200`**).
