...
`LastNameR1(config)#ip access-list standard BLOCK_FB

LastNameR1(config-std-nacl)#remark ACL to block Facebook traffic for VLAN 1

LastNameR1(config-std-nacl)#remark Deny traffic to known Facebook CIDR ranges

LastNameR1(config-std-nacl)#remark Permit all other traffic

LastNameR1(config-std-nacl)#deny 31.13.64.0 0.0.63.255

LastNameR1(config-std-nacl)#deny 157.240.0.0 0.0.255.255

LastNameR1(config-std-nacl)#permit any`
...
`LastNameR1(config)#interface fa0/0.1

LastNameR1(config-if)#ip access-group BLOCK_FB out`

![Image](assets/images/file-674628b50f369.png)
...
View the ACL to confirm matches on the `deny` statements:
 ...
 `LastNameR1#show access-lists BLOCK_FB `
...
![Image](assets/images/file-6745f4662301f.png)
