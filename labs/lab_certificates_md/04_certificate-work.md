...
![Excerpt from the OpenSSL CA_default configuration showing the CA directory, database, certificate, serial, and private key paths.](./assets/images/file-5f32b61530c21_slide2_flat.png)
...
![openssl req -new -x509 command generating the CA certificate and encrypted private key.](./assets/images/file-5f32054c3c4a4.png)

![Directory listing showing cacert.pem and cakey.pem created in the CA root home directory.](./assets/images/file-5f32060ef11e3.png)

This creates two files.

Certificate files, private keys, and CSRs usually include recognizable headers and footers.

![head output showing BEGIN CERTIFICATE in cacert.pem and BEGIN ENCRYPTED PRIVATE KEY in cakey.pem.](./assets/images/file-5f3206b825097.png)
...
![file and cat output comparing cacert.pem and cakey.pem and illustrating why file type alone is not enough.](./assets/images/file-5f3207412a006.png)
...
![Long listing confirming cacert.pem has been moved into /etc/ssl/certs.](./assets/images/file-5f320be305fd4_slide9_flat.png)
...
```
![Terminal running the openssl req command that generates the web server CSR and private key.](./assets/images/Pasted%20image%2020260304102842.png)

Two files will have been created
![Directory listing showing the newly created CSR file and Apache private key.](./assets/images/Pasted%20image%2020260304102934.png)


Verify the files are as you would expect
![Terminal showing the headers of the CSR and private key files with BEGIN CERTIFICATE REQUEST and BEGIN PRIVATE KEY.](./assets/images/Pasted%20image%2020260304103001.png)
...
![OpenSSL text view of the CSR showing the subject fields and public key information.](./assets/images/file-5f3213391cd90.png)
...
![Terminal output of cat csrdetails showing the OpenSSL request configuration and subjectAltName entries.](./assets/images/Pasted%20image%2020260304102815.png)
...
![Terminal on the LAMP server preparing the CSR file so it can be transferred to the CA host.](./assets/images/file-5f32199799cdf.png)
...
![scp command on the CA server copying the CSR from the LAMP host and prompting to trust the remote SSH key.](./assets/images/file-5f321b57c2dd3.png)

**Question**: Why we had this error?

File should now be in the right location.

![Long listing in /etc/ssl/certs confirming the CSR file is now on the CA server.](./assets/images/file-5f321b9634173_slide6_flat.png)
...
![cat output showing the BEGIN CERTIFICATE REQUEST block from the CSR stored in /etc/ssl/certs.](./assets/images/file-5f321d5b446c4_slide7_flat.png)
...
![openssl ca command output prompting for the CA key passphrase and asking whether to sign the certificate.](./assets/images/file-5f3222d84302f_slide1_flat.png)
...
![Terminal showing the CA serial file incremented and index.txt updated after issuing a certificate.](./assets/images/Pasted%20image%2020260304104846.png)
...
![OpenSSL duplicate-request error showing the certificate database rejected signing the same CSR again.](./assets/images/file-5f3227b7d9f5b_slide2_flat.png)
...
![scp command on the LAMP server copying 01.pem from the CA into /etc/ssl/certs as the signed certificate.](./assets/images/file-5f322a1914417_slide3_flat.png)
